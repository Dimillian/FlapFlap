...
{
    SKNode *topPipe = [self childNodeWithName:kTopPipeName];
    SKNode *bottomPipe = [self childNodeWithName:kBottomPipeName];
    if (topPipe && bottomPipe) {
        if ([topPipe intersectsNode:_player] || [bottomPipe intersectsNode:_player]) {
            [self gameOver];
        }
    }
